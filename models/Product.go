...
func GetProductByPage(pageNum,num int, category string) interface{}{
	...
	for i := 0; i < len(product); i++{
		//查询用户对应的角色
		o.Raw("SELECT b.id as fileid,file_address FROM `product` a inner join `file` b WHERE a.id = ?",product[i].Id).Values(&maps)
		product[i].Fileinfo.File_Id = maps[0]["fileid"].(string)
		product[i].Fileinfo.File_address = util.GetIntranetIp()+maps[0]["file_address"].(string)
	}
	//data
...
//插入产品
func InsertProduct(category,model,name,specal,introduce,file_id string) int{
	//初始化日期
	//publish_date = time.Now().Format("2006-01-02")//时间戳"2006-01-02 15:04:05"是参考格式,具体数字可变
	//判空前端做
	o := orm.NewOrm()
	o.Using("default")
	_,err := o.Raw("insert `product`(category,model,name,specal,introduce,file_id) value(?,?,?,?,?,?)",category,model,name,specal,introduce,file_id).Exec()
	if err == nil {
		return 1
	}
	fmt.Println("产品插入失败")
...
//更新产品,注：提交相同数据返回行数为0
func UpdateProduct(id,category,model,name,specal,introduce,file_id string) int64{
	var num int64;//返回影响的行数
	//判空前端做
	o := orm.NewOrm()
	o.Using("default")
	res,err := o.Raw("update `product` set category=?,model=?,name=?,specal=?,introduce=?,file_id=? where id=?",category,model,name,specal,introduce,file_id,id).Exec()
	if err == nil {
		num, _ = res.RowsAffected()
	}
	fmt.Println("产品更新失败")
